...
# Используйте цвет для разделения графиков. Добавьте текстовые подписи
# величин импорта и экспорта. Отрегулируйте местоположение и размер шрифта
...
p <- p + geom_text(aes(x = year, y = value, label = value), nudge_y = 0.02)
